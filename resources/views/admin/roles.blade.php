<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Managed Role Permissions') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('admin-panel.permissions.store') }}" class="bg-white p-6 shadow-sm sm:rounded-lg">
                @csrf
            ...
            @foreach ($roles as $role)
                <form method="POST" action="{{ route('admin-panel.roles.permissions.update', $role) }}" class="bg-white p-6 shadow-sm sm:rounded-lg">
                    @csrf
                    @method('PUT')
